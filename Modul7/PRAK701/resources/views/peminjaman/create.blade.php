...
@section('content')
    <div class="mx-auto max-w-3xl">
        <div class="mb-6">
            <h2 class="text-2xl font-bold text-slate-900">Tambah Peminjaman</h2>
            <p class="mt-1 text-sm text-slate-600">Isi transaksi peminjaman baru.</p>
        </div>
            ...
            <form action="{{ route('peminjaman.store') }}" method="POST" class="space-y-5">
                ...
                <div class="grid gap-5 sm:grid-cols-2">
                    <div>
                        ...
                        <input id="tgl_pinjam" name="tgl_pinjam" type="date" value="{{ old('tgl_pinjam') }}" class="w-full rounded-xl border border-slate-300 px-4 py-3 outline-none transition focus:border-brand-500 focus:ring-4 focus:ring-brand-100 @error('tgl_pinjam') border-red-400 focus:border-red-500 focus:ring-red-100 @enderror" required>
                    ...
                    <div>
                        ...
                        <input id="tgl_kembali" name="tgl_kembali" type="date" value="{{ old('tgl_kembali') }}" class="w-full rounded-xl border border-slate-300 px-4 py-3 outline-none transition focus:border-brand-500 focus:ring-4 focus:ring-brand-100 @error('tgl_kembali') border-red-400 focus:border-red-500 focus:ring-red-100 @enderror" required>
                ...
                <div>
                    ...
                    <select id="id_member" name="id_member" class="w-full rounded-xl border border-slate-300 px-4 py-3 outline-none transition focus:border-brand-500 focus:ring-4 focus:ring-brand-100 @error('id_member') border-red-400 focus:border-red-500 focus:ring-red-100 @enderror" required>
                        ...
                        @foreach ($members as $member)
                            <option value="{{ $member->id_member }}" @selected(old('id_member') == $member->id_member)>{{ $member->nomor_member }} - {{ $member->nama_member }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="id_buku" name="id_buku" class="w-full rounded-xl border border-slate-300 px-4 py-3 outline-none transition focus:border-brand-500 focus:ring-4 focus:ring-brand-100 @error('id_buku') border-red-400 focus:border-red-500 focus:ring-red-100 @enderror" required>
                        ...
                        @foreach ($bukus as $buku)
                            <option value="{{ $buku->id_buku }}" @selected(old('id_buku') == $buku->id_buku)>{{ $buku->judul_buku }} - {{ $buku->penulis }}</option>
                        @endforeach
                    </select>
